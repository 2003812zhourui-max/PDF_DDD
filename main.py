...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--start-time", default=DEFAULT_START_TIME, help="下载开始时间，例如 2026-06-01 00:00:00")
    parser.add_argument("--end-time", default=DEFAULT_END_TIME, help="下载结束时间，例如 2026-06-02 23:59:59")
    parser.add_argument("--wh-codes", default=DEFAULT_WH_CODES, help="仓库代码，多个用英文逗号分隔")
    ...
    parser.add_argument("--output-name", default=DEFAULT_OUTPUT_NAME, help="输出文件名前缀")
    ...
    parser.add_argument("--limit", type=int, default=DEFAULT_LIMIT, help="抽样处理数量，0 表示不限")
    ...
    parser.add_argument("--username", default=DEFAULT_WMS_USERNAME, help="WMS 登录账号（HTTP 并发模式需要）")
    parser.add_argument("--password", default=DEFAULT_WMS_PASSWORD, help="WMS 登录密码（HTTP 并发模式需要）")
    parser.add_argument("--workers", type=int, default=DEFAULT_WORKERS, help="HTTP 并发下载线程数，默认 5")
    ...
    parser.add_argument("--browser-mode", action="store_true", default=DEFAULT_BROWSER_MODE, help="使用旧浏览器模式下载（需要 Playwright）")
