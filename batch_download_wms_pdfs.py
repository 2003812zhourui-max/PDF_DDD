...
from config import (
    ...
    DEFAULT_WH_CODES,
    DEFAULT_WMS_PASSWORD,
    DEFAULT_WMS_USERNAME,
    DEFAULT_WORKERS,
    WMS_BASE_URL,
    WMS_TARGET_PAGE,
    config_bool,
...
TARGET_PAGE = WMS_TARGET_PAGE
API_ORIGIN = WMS_BASE_URL
...
PDF_DIR = DEFAULT_INPUT_DIR
LOG_FILE = DEFAULT_DOWNLOAD_LOG
AUTH_STATE_FILE = DEFAULT_STORAGE_STATE
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--current", type=int, default=config_int("download", "current_page", default=1), help="起始页码，默认 1")
    parser.add_argument("--size", type=int, default=config_int("download", "page_size", default=100), help="每页数量，默认 100")
    parser.add_argument("--max-pages", type=int, default=config_int("download", "max_pages", default=10), help="最大页数，默认 10")
    parser.add_argument("--total-limit", type=int, default=DEFAULT_LIMIT or 1000, help="最多处理订单数，默认 1000")
    parser.add_argument("--headless", action="store_true", default=config_bool("browser", "headless", default=False), help="启用无头模式；默认 headless=False")
    parser.add_argument("--date", default="", help="目标日期 YYYY-MM-DD；不传则默认昨天")
    parser.add_argument("--start-time", default=DEFAULT_START_TIME, help="创建时间起始；优先级高于 --date")
    parser.add_argument("--end-time", default=DEFAULT_END_TIME, help="创建时间结束；优先级高于 --date")
    parser.add_argument("--wh-code", default=str(get_config("download", "wh_code", default="")), help="列表查询仓库编码；不传时优先使用登录态里的当前仓库")
    parser.add_argument("--wh-codes", default=DEFAULT_WH_CODES, help="多个仓库编码，逗号分隔，例如 Monrovia,US02")
    parser.add_argument("--wh-codes-file", default="", help="仓库编码文件，每行一个 whCode")
    parser.add_argument("--statuses", default=DEFAULT_STATUSES, help="WMS 状态码，默认 15；待处理通常是 10")
    ...
    parser.add_argument("--username", default=os.environ.get("WMS_USERNAME") or DEFAULT_WMS_USERNAME, help="WMS 登录账号，默认读取 WMS_USERNAME")
    parser.add_argument("--password", default=os.environ.get("WMS_PASSWORD") or DEFAULT_WMS_PASSWORD, help="WMS 登录密码；不推荐使用，容易留在命令历史")
    ...
    parser.add_argument("--warehouse", default=str(get_config("download", "warehouse", default="Monrovia")), help="自动选择的仓库，默认 Monrovia")
    parser.add_argument("--validate-pdf", action="store_true", default=config_bool("download", "validate_pdf", default=True), help="下载后检查 PDF 页面尺寸，默认开启")
    parser.add_argument("--no-validate-pdf", dest="validate_pdf", action="store_false", help="关闭 PDF 页面尺寸检查")
    parser.add_argument("--expected-formats", default=str(get_config("download", "expected_formats", default="4x6")), help="允许的 PDF 面单格式，逗号分隔，默认 4x6")
    ...
    parser.add_argument("--force", action="store_true", default=config_bool("download", "force", default=False), help="即使下载日志已有 success，也重新下载")
    parser.add_argument("--channel", default=DEFAULT_CHANNEL, help="物流渠道筛选，例如 TikTok-CBT-US、Upload_Shipping_Label-Speedx")
